...
public class DeviceBatteryRechargeRecorder {
	...
	private static Boolean recordModified = false;
	private static Boolean showOnce = false;
	private static Boolean lastLoginModified = false;

	public static void main(String[] args) {
		final String[] mainMenu = {"\nI wish to:\n",
			"1) Create a new recharge record for the current device\n",
			"2) Create a new recharge record for a different device\n",
			"3) Finish last recharge record for the current device\n",
			"4) Finish last recharge record for a different device\n",
			"5) Search for record(s)\n",
			"6) Export reocrds\n",
			"7) Import reocrds\n",
			"8) Display information\n",
			"9) Update a record\n",
			"10) Delete records\n",
			"11) Switch to another user's device\n",
			"12) Quit\n",
			"Enter your choice: "};
		int choice = -1;

		login();
		devices = giveTotalsOfDeviceUnderUser(userName);
		while (choice != 12) {
	...
	private static void login () {
		...
		if (lastLogin.exists()) {
			...
			if (user != null && device != null) {
				System.out.println("Logged in as " + user + "'s " + device + ".");
				userName = user;
				deviceName = device;	
			}
			else // in case file wrongly written by accident or otherwise
				needAsk = true;
		} 
			...
			try  {
				...
				if (recs.length > 0) {
					for (Record rec : recs) {// collect substantial records from recs
						if (rec == null) 
							break;
						if (verifyRecordIntegrity(rec) == false) {
							System.out.println("Record integrity test failed. Data corrupted.");
							System.exit(1);
						}
						records.add(rec);
					} // The statement below is a silent internal search for current user's current device.
					//displayRecords(records, false, false);
	...
	private static void switchCurrentUserAndDevice () {
	...
	private static void askUserToAddAccount () {
		...
		System.out.print("Start a new account and create your first record of device battery recharge? (Y/n): ");
	...
	private static Boolean verifyRecordIntegrity (Record rec) {
		return (rec.getUserName() != null && rec.getDeviceName() != null && rec.getStart() != null && rec.getStartPercent() != 0);
	}
	private static void addRecord (Boolean internal) {
		...
		if (!internal) {
			if (devices != null) {
				System.out.println("Choose the device to add a recharge record:");
				int i;
		...
		System.out.println("Now leave your device on recharge and come back to finish your record later.");
		records.add(record);
		recordModified = true;
	}
	private static void finishRecord (Boolean internal) {
		...
		if (unfinishedRecs.size() > 0) {
			if (internal) { // The statement below is a silent internal search for current user's current device.
				...
				displayRecords(unfinishedRecs, false, false);
				System.out.print("Which record you'd like to finish? Or enter 'q' to quit: ");
	...
	private static void searchRecords () {
		final String[] options = {"User name", "Device name", "A range of start dates", 
									"A device under current user name",
									"A device under other user name", 
									"A start date range under a user name", "A start date range for a device",
									"A start date range for a device under a user name", 
									"A range of start or finished percents under a user name",
									"A range of start or finished percents for a device", 
									"A range of start or finished percents for a device under a user",
									"A range of charged percents under a user name",
									"A range of charged percents for a device",
									"A range of charged percents for a device under a user name",
									"Return"
								};
		final int[] frequentOptions = {1, 2, 3, 4, 5, 6, 7, options.length};
		...
		for (int i = 1; i <= frequentOptions.length; i++) 
			hm.put(i, frequentOptions[i-1]);
		while (option != QUIT) {
			try {
				System.out.println("\nSearch by: ");
				for (int i = 1; i <= frequentOptions.length; i++) 
					System.out.println(i + ") " + options[hm.get(i)-1]);
				System.out.print("Enter your choice (enter 0 to see the full list of options): ");
				option = in.nextInt();
				in.nextLine();
				if (option == 0) {
			...
			if (option != QUIT)
				if (results != null && results.size() > 0) {
					if (shorter)
						System.out.println("Found records for " + results.get(0).getUserName() + "'s " + results.get(0).getDeviceName() + ":");
					displayRecords(results, false, shorter);
				} else
	...
	private static ArrayList<Record> searchByUser (List<Record> recs, String name) {
		...
		int index = firstIndexOf(recsForQuery, query, sbun); 
		int [] ends = null;
		if (index >= 0) {
	...
	private static ArrayList<Record> searchByDevice (List<Record> recs, String device) {
		...
		int index = firstIndexOf(recsForQuery, query, sbdn);
		int [] ends = null;
		if (index >= 0) {
	...
	private static ArrayList<Record> searchByStartDateRange (List<Record> recs) {
		final String[] inputGuide = {	"* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * \n",
										"* For particular day in a month in a year, e.g. 18/8/2020 or 2020/8/18                                                          *",
										"* For a day in a month in the current year, e.g. 15/4, September 19, or 8 Feb                                                   *",
										"* For a day in current month, just enter the day, e.g. 20                                                                       *\n",
										"* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *\n",
										"* For a range of days across years, e.g. 20/09/2020-14/05/2021, or 2020/09/20-2021/05/14                                        *",
										"* For a range of days within a year, e.g. 13/2-22/06/2020, or 2020/2/13-06/12, or May 20-July 20, 2020 or 20 May-20 July, 2020  *",
										"* For a range of days within a month, e.g. 1-21/05/2020, 2020/05/1-21                                                           *", 
										"* For a range of days across months in current year, e.g. Mar 10-June 20, or 10 March-10 April                                  *",
										"* For a range of days within a month of current year, e.g. 3-25/06                                                              *",
										"* For a range of days within current month, e.g. 2-30                                                                           *\n",
										"* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * \n",
										"* For a month in a year, e.g. August, 2020, or Oct, 2020 or 2020/4, or 5/2020                                                   *",
										"* For current year, just enter the month, e.g. July, or Aug. No digits should be used for representing month alone              *",
										"* Free standing digits are used for representing a day of a month                                                               *\n",
										"* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * \n",
										"* For a of month range, e.g. Feb-May, 2020, or 4-8/2020, or 2020/4-8                                                            *",
										"* For current year, just enter a month range, e.g. March-June, Apr-Sep                                                          *\n",
										"* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * \n",
										"* For a particular year, e.g. 2020                                                                                              *",
										"* For a range of years, e.g. 2020-2021, 2020-22                                                                                 *\n",
										"* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * \n",
										"* Enter 0 to return                                                                                                             *\n",
										"* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * \n"
									};
		System.out.println("\nEnter a date range (enter 'i' to see input guide): ");
		String dateString = getStringInput(true);
		if (dateString.equals("I")) {
			for (String s : inputGuide) 
				System.out.println(s);
			System.out.println("\nEnter a date range: ");
			dateString = getStringInput(true);
		}
	...
	private static int[] getStartDateIndexBounds (Record[] recs, String queryString) {
		...
		Date date = new GregorianCalendar(yMD[0], (yMD[1] - 1), yMD[2]).getTime(); // dateString 
		Record queryDate = new Record(date); // here only date is of interest
		index = firstIndexOf(recs, queryDate, sbsd); 
		if (index >= 0) 
	...
	private static void modifyRecords (List<Record> recs, UserDeviceStartCAU udsc) {
				...
				if (itemsForMod.size() == 1)
					...
					if (option.equals("Y")) 
						...
						for (int i : itemsForMod) {//index in fndRecs starts at 0, so it should be i - 1
							...
							if (!success) {
								System.out.println("An unsuccessful modification occurred. Not all records recordModified.");
								break;
							}
						} 	
					else 
		...
		if (success) {
			System.out.println("Device information has been recordModified.");
			recordModified = true;
		}
		else 
	...
	private static Boolean modifyRecord (Record rec, int index, UserDeviceStartCAU udsc) {
		...
		System.out.println(getDisplayString(rec, index, false, false));
		if(udsc == UserDeviceStartCAU.DEVICE) {
			...
			System.out.print("Enter the new state of CAU (enter T for \"true\" or F for \"false\" or q to quit): ");
			...
			System.out.println("Modify: ");
			System.out.print("(Y)ear, (M)onth, (D)ay,  (H)our, mi(N)ute, or (S)econd? (enter q to quit): ");
			response = getStringInput(true);
			if (!response.toLowerCase().equals("q")) {
				Date newDate = updateDate(rec.getStart(), response, false);
				System.out.println(newDate);
				if (newDate != null) {
					rec.setStart(newDate);
					success = true;
				}
			}
		}
	...
	private static Boolean batchModifyRecords (List<Record> recs, List<Integer> items, UserDeviceStartCAU udsc) {
		...
		if (udsc == UserDeviceStartCAU.DEVICE) {
			...
			System.out.print("Enter the new use name (q to quit): ");
			user = getStringInput(false);
			...
			System.out.print("Enter the new CAU state (enter T for \"true\" or F for \"false\" or q to quit): ");
			...
			System.out.println("Modify: ");
			System.out.print("(Y)ear/(M)onth/(D)ay or (H)our:mi(N)ute:(S)econd? (enter q to quit): ");
			start = getStringInput(true);
			if (!start.equals("Q")) {
				update = updateDate(recs.get(0).getStart(), start, true);
				if (update == null) {
					System.out.println("An invalid input encountered. Abort updates.");
					return false;
				}
				for (int i : items)
					recs.get(i-1).setStart(update);
				showOnce = false; // Job done. Throw back to original state for next job.
				success = true;
			}
		}
		return success;
	}
	private static Date updateDate (Date date, String response, Boolean batch) {
		...
		if (entry >= 0) {
			...
			GregorianCalendar cal = new GregorianCalendar(dateComponents[0], dateComponents[1]-1, dateComponents[2], 
	...
	private static String getModificationOption (List<Record> recs) {
		String entry;
		displayRecords(recs, false, false);
	...
	private static void deleteRecords () {
		...
		System.out.print("Delete records of current user? (Y/n): ");
		if (getStringInput(true).equals("Y"))
			name = userName;
		else {
		...
		ArrayList<Record> fndRecs = searchByUser(records, name);
		if (fndRecs == null) {
			System.out.println("No record(s) found for " + name);
			return;
		}
		System.out.println("WARNING: This will delete all records whose charging time exceeds " + LIMIT + " hours (inclusive), ");
		System.out.println("or which didn't finish recharge in the last " + LIMIT + " hours. ");
		ArrayList<Record> toRemove = new ArrayList<>();
		for (Record r : fndRecs)
			if (r.getStart() != null && r.getFinish() == null) {
				timeDiff = getTimeDifference((new Date()).getTime() - r.getStart().getTime());
				if (timeDiff[0] < 0 || timeDiff[0] >= LIMIT)
					toRemove.add(r);
			}
			else if (r.getStart() == null) {
				System.out.println("Null record encountered.");
				toRemove.add(r);
			}
		System.out.println("Records to be removed:");
		displayRecords(toRemove, false, false);
		System.out.print("You sure you want delete the above records? (Y/n): ");
		if (getStringInput(true).equals("N"))  {
	...
	private static Boolean exportToLocal (List<Record> recs) {
		...
		try (PrintWriter pw = new PrintWriter(new FileOutputStream(new File("output.txt")))) {
			for (Record r : recs) {
				pw.println(getDisplayString(r, i, true, false));
				i += 1;
			}
		} catch (FileNotFoundException fnfe) {
			fnfe.printStackTrace();
	...
	private static Boolean exportToNet (List<Record> recs) {
		final int PORT = 1234;
		final String address = "192.168.1.195";
		InetAddress host;
		Socket link = null;
		int recCnt = 0;
	...
	private static ArrayList<Record> importFromNet () {
		...
		try {
			...
			do {
				line = input.nextLine();
				if (line.equals("***CLOSE***") || line.length() == 0)
					break; 
				rec = parseRecordString(line);
				if (verifyRecordIntegrity(rec))
					imports.add(rec);
				else 
					System.out.println("#" + (++i) + " record corrupted.");
				numMsg++;
			} while (true);
	...
	private static Record parseRecordString (String recStr) {
		final int[] possibleLengths = {4,5,8,9};
		...
		int startPercent, finishPercent;
		Boolean afterUse;

		userName = recordComponents[0];
		deviceName = recordComponents[1];
		dateComponentsStart = getDateComponents(recordComponents[2], " ");
		...
		startPercent = Integer.parseInt(recordComponents[3]);
		if (recordComponents.length == 4) 
			record = new Record(userName, deviceName, startDate, startPercent);
		else if (recordComponents.length == 5) {
			afterUse = (recordComponents[4] == "true")? true : false;
			record = new Record(userName, deviceName, startDate, startPercent, afterUse);
		}
		else if (recordComponents.length == 8) {
			dateComponentsStart = getDateComponents(recordComponents[2], " ");
			dateComponentsFinish = getDateComponents(recordComponents[4], ":");
			...
			finishPercent = Integer.parseInt(recordComponents[5]);
			record = new Record(userName, deviceName, startDate, startPercent, finishDate, finishPercent);
		}
		else if (recordComponents.length == 9) {
			dateComponentsStart = getDateComponents(recordComponents[2], " ");
			afterUse = (recordComponents[4] == "true")? true : false;
			dateComponentsFinish = getDateComponents(recordComponents[5], ":");
			finishDate = new GregorianCalendar(dateComponentsStart[0], dateComponentsStart[1], dateComponentsStart[2], 
										dateComponentsFinish[0], dateComponentsFinish[1], dateComponentsFinish[2]).getTime();
			finishPercent = Integer.parseInt(recordComponents[6]);
			record = new Record(userName, deviceName, startDate, startPercent, afterUse, finishDate, finishPercent);
		}	
		return record;
	}
	...
	private static void displayRecords (List<Record> recs, Boolean internal, Boolean shorter) {
		int i = 1;
		for (Record r : recs) {
			System.out.println(getDisplayString(r, i, internal, shorter));
			i += 1;
		} 
	}
	private static String getDisplayString (Record r, int index, Boolean internal, Boolean shorter) {
		if (r == null || verifyRecordIntegrity(r) == false) {
			System.out.println("Corrupted record encountered.");
			return null;
		}
		...
		if (internal) {
			...
			if (afterUse != null) 
				startString = startString + "|" + afterUse;
		}
		else {
			startString = "s: " + start + " @: " + startPercent + "%";
			if (afterUse != null)
				startString = startString + " cau: " + afterUse;
			if (!shorter)
				startString =  user + "'s " + device + " " + startString;
		}
		if (r.getFinish() == null)
			result = startString;
		else {
	...
	private static ArrayList<String> giveTotalsOfDeviceUnderUser (String user) {
		ArrayList<Record> fndRecs = searchByUser(records, user);
		if (fndRecs == null)
			return null;
		Record[] recs = fndRecs.toArray(new Record[fndRecs.size()]);
		SortByDeviceName sbdn = new SortByDeviceName();
		Arrays.sort(recs, sbdn);
		ArrayList<String> devices = new ArrayList<>();
		ArrayList<Date> firstDates = new ArrayList<>();
		String dev = recs[0].getDeviceName();
		Date date = recs[0].getStart();
		devices.add(dev); // Add first new device name to the ArrayList
		firstDates.add(date);
		// indexs of the first of each different device group in the record list
		ArrayList<Integer> indexes = new ArrayList<>(); 
		indexes.add(0);
		int i;
		for (i = 1; i < recs.length; i++) 
			// If another device encountered, add it to the devices ArrayList.
			if (!fuzzyStringMatch(dev, recs[i].getDeviceName())) { 
				dev = recs[i].getDeviceName();
				date = recs[i].getStart();
				devices.add(dev);
				firstDates.add(date);
				indexes.add(i);
			}
		Date now = new Date(); 
		long days = (now.getTime() - firstDates.get(0).getTime()) / (24 * 3600 * 1000);
		System.out.println("Total number of records of " + user + ": " + fndRecs.size() +
							" in " + days + " days.");
		ArrayList<Integer> diffs = new ArrayList<>(indexes.size());
		for (i = 0; i < indexes.size() - 1; i++)
			diffs.add((indexes.get(i+1) - indexes.get(i)));
		// number of records for last different device in the list
		diffs.add(fndRecs.size() - indexes.get(indexes.size()-1)); 
		for (i = 0; i < devices.size(); i++) {
			days = (now.getTime() - firstDates.get(i).getTime()) / (24 * 3600 * 1000);
			System.out.println(devices.get(i) + ": " + diffs.get(i) + " in " + days + " days.");
		}
		return devices;
	}
	private static <T> int firstIndexOf (T[] a, T key, Comparator<? super T> c) {
		int index = -1;
	...
	private static void quit (String bye) {
		...
		if (recordModified) {
			for (Record rec : records) {
				if (verifyRecordIntegrity(rec) == false) {
					System.out.println("Record integrity test failed. Data corrupted.");
					System.out.println("Record data file not written.");
					System.exit(1);
				}
			}
...
class Record implements Serializable {
	...
	public Record (String userName, String deviceName, Date start, int startPercent, Boolean afterUse) {
		this.userName = userName;
		this.deviceName = deviceName;
		this.start = start;
		this.startPercent = startPercent;
	...
	public Record (String userName, String deviceName, Date start, int startPercent, Date finish, int finishPercent) {
		this.userName = userName;
		this.deviceName = deviceName;
		this.start = start;
		this.startPercent = startPercent;
	...
	public Record (String userName, String deviceName, Date start, int startPercent, Boolean afterUse, Date finish, int finishPercent) {
		this.userName = userName;
		this.deviceName = deviceName;
		this.start = start;
		this.startPercent = startPercent;
